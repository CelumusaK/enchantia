...
func Enter():
	...
	anim["parameters/Locomotion/conditions/sprint"] = true
	
func Exit():
	print("Exit Sprint")
	anim["parameters/conditions/locomotion"] = false
	anim["parameters/Locomotion/conditions/sprint"] = false
...
func Physics_Update(delta: float):
	...
	if Input.is_action_pressed("run") and not Input.is_action_pressed("sprint") and player.velocity != Vector3.ZERO:
		Transitioned.emit(self, "Running")
		
	if not Input.is_action_pressed("sprint") and player.velocity != Vector3.ZERO:
